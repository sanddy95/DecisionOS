  ...
  return (
    <div className="p-8 space-y-8 max-w-4xl">
      <div>
        ...
        <p className="text-sm text-gray-400 mt-1">Subscription plans, LLM providers, and feature flags</p>
      </div>

      {/* Subscription plans */}
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="font-semibold text-gray-200">Subscription Plans</h2>
          <p className="text-xs text-gray-500">LLM provider access is enforced per plan</p>
        </div>
        <div className="space-y-3">
          {subscriptionPlans.map(plan => (
            <div key={plan.id} className={`bg-gray-900 border border-gray-800 border-l-4 ${colorBorders[plan.color]} rounded-xl p-5`}>
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-3 mb-3">
                    <h3 className={`font-bold text-lg ${colorText[plan.color]}`}>{plan.name}</h3>
                    <span className="text-gray-300 font-semibold">{plan.priceMonthly > 0 ? `$${plan.priceMonthly}/mo` : 'Free'}</span>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {plan.allowedProviders.map(p => (
                          <span key={p} className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] bg-gray-800 text-gray-300 border border-gray-700">{p}</span>
                        ))}
                      </div>
                    </div>
                  </div>
                </div>
                <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs bg-gray-800 text-gray-300 hover:bg-gray-700 transition-colors border border-gray-700">
                  <Pencil size={11} /> Edit
                </button>
              </div>
            </div>
      ...
      {/* Feature flags */}
      <div className="space-y-4">
        <h2 className="font-semibold text-gray-200">Feature Flags</h2>
          ...
          {flags.map(flag => (
            <div key={flag.key} className="flex items-center justify-between px-5 py-4">
              ...
              <button
                ...
                className={`w-11 h-6 rounded-full transition-colors relative shrink-0 ${flag.enabled ? 'bg-violet-600' : 'bg-gray-700'}`}>
